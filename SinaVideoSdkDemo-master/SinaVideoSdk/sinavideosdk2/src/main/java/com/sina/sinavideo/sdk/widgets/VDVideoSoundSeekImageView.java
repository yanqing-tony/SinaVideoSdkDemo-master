...
public final class VDVideoSoundSeekImageView extends ImageView implements
	...
	public VDVideoSoundSeekImageView(Context context, AttributeSet attrs) {
		...
		TypedArray typedArr = context.obtainStyledAttributes(attrs,
				R.styleable.VDVideoSoundSeekImageView);
		if (typedArr != null) {
			for (int i = 0; i < typedArr.getIndexCount(); i++) {
				if (typedArr.getIndex(i) == R.styleable.VDVideoSoundSeekImageView_muteSrc) {
					...
					if (mMuteDrawable == null) {
						mMuteDrawable = context.getResources().getDrawable(
								R.drawable.play_ctrl_sound_gestrue_silent);
					}
				}
			}
			if (mMuteDrawable == null) {
		...
		if (typedArr != null) {
			...
			if (mSrcDrawable == null) {
				mSrcDrawable = context.getResources().getDrawable(
						R.drawable.play_ctrl_sound_gestrue);
			}
			typedArr.recycle();
		}
	}
